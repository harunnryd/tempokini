...
import (
	...
	"github.com/harunnryd/tempodoloe/internal/app/handler"
	"github.com/harunnryd/tempodoloe/internal/pkg/chicustom"
)
...
func (s *server) Router(handler handler.Handler) (w chicustom.Router) {
	...
	w.Route("/v1", func(r chi.Router) {
		...
		router.Action(chicustom.NewRest(http.MethodGet, "/products/{id}", handler.Product().GetProductByID))
		router.Action(chicustom.NewRest(http.MethodPatch, "/products/{id}/deduct", handler.Product().DeductProductQuantityByID))
	})
